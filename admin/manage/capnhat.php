...
if (isset($_GET['id'])) {
    ...
    if (isset($_POST['capnhat']) && !empty($_POST)) {
        if ((isset($_POST['ma']) &&
                ...
                isset($_FILES['img']) && isset($_FILES['img']['name'])) &&
            (!empty($_POST['ma']) &&
                !empty($_POST['ten']) &&
                !empty($_POST['so_luong']) &&
                !empty($_POST['noi_dung']) &&
                !empty($_POST['phan_loai']) &&
                !empty($_FILES['img'])) && !empty($_FILES['img']['name'])
        ) {
            ...
            //    var_dump($query);die;
            header('location: http://localhost/web_ban_hang/admin/index.php?page=tatcasanpham');
            die;
        }
    } else {
        $_SESSION['kt'] = "Xin mời kiểm tra lại!";
    }
    // var_dump($data);die;
}
?>
